...
public class MethodProcessor {
    private static final List<BaseProcessor> compilers = new ArrayList<>();
    ...
    static {
        ReflectionUtils.getClasses("ru.itskekoff.j2c.translator.processor.cpp.impl", BaseProcessor.class)
                .forEach(processor -> {
                    TranslatorMain.LOGGER.info("Added processor: {}", processor.getClass().getSimpleName());
                    compilers.add(processor);
                });
    }

    public void process(MethodNode method, ClassNode clazz, ClassContext writer, NativeLinker nativeLinker) {
        ...
        if (!isClinit(method)) nativeLinker.pushMethod(method, nativeName);
        ...
        writer.output().pushMethod(method, nativeName, argNames, CPP_TYPES, args, isClinit(method), isStatic);
        writer.output().pushMethodLine("// stack count: %d, locals count: %d, try-catches: %d"
                .formatted(method.maxStack, method.maxLocals, method.tryCatchBlocks.size()));

        if (method.maxStack == 0) {
            writer.output().pushMethodLine("jvalue cstack_exception = {};");
        } else {
            ...
            writer.output().pushMethodLine(stackBuilder.toString());
            writer.output().pushMethodLine(localBuilder.toString());
            writeLocals(writer, argTypes, argNames);
        }
        ...
        for (TryCatchBlockNode tryCatchBlock : method.tryCatchBlocks) {
            ...
            String targetLabel = writer.getLabelPool().getName(tryCatchBlock.handler.getLabel());
            catchHandlers.computeIfAbsent(catchLabel, _ -> new ArrayList<>())
                    .add(new CatchInfo(tryCatchBlock.type, targetLabel));
        }

        processInstructions(method, writer, catchLabels);
        writeCatchHandlers(writer, method, catchHandlers);

        finalizeMethod(method, writer);
    }
    ...
    private void writeLocals(ClassContext writer, List<Type> argTypes, List<String> argNames) {
        if (!argTypes.isEmpty()) {
    ...
    private void processInstructions(MethodNode method, ClassContext writer, Map<LabelNode, String> catchLabels) {
        if (TranslatorConfiguration.IMP.MAIN.DEBUG_ENABLED) TranslatorMain.LOGGER.info("Processing method {}{}", method.name, method.desc);
        ...
        method.instructions.forEach(insn -> {
            ...
            compilers.stream()
                    ...
                    .ifPresentOrElse(
                            ...
                            () -> TranslatorMain.LOGGER.error("Unsupported insn {} [{}]",
                                    BaseUtils.getOpcodeString(insn.getOpcode()),
                                    insn.getOpcode())
                    );
    ...
    private void writeCatchHandlers(ClassContext writer, MethodNode method, Map<String, List<CatchInfo>> catchHandlers) {
        catchHandlers.forEach((catchLabel, handlers) -> {
            writer.output().pushLine().pushString("%s:".formatted(catchLabel));
        ...
        writer.output().pushLine().pushString("L_EXCEPTION: if (env->ExceptionOccurred()) { env->ExceptionDescribe(); env->ExceptionClear(); %s }\n"
    ...
    private void finalizeMethod(MethodNode method, ClassContext writer) {
